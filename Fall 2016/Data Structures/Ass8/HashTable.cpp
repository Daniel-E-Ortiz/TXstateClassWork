...
bool HashTable::search(const char* cStr) const
{
   ...
   size_type i = 0;
   while(strcmp(data[(key + (i*i))%capacity].word, cStr) != 0)
   {
      if (i == capacity)
         return false;
      ++i;
...
void HashTable::insert(const char* cStr)
{
   if (load_factor() > 0.45)
      rehash();
   size_type key = hash(cStr) % capacity;
   if (strcmp(data[key%capacity].word,"") == 0)
      strcpy(data[key].word, cStr);
   else
   {
      size_type i = 1;
      while(strcmp(data[(key + (i*i))%capacity].word,"") != 0)
         ++i;
      strcpy(data[(key + (i*i))%capacity].word,cStr);
   }
   ++used;
}
