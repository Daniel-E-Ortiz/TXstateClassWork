...
void CopySingletonsGatherDups (Node* head1, Node*& head2)
{
   ...
   {  
      ...
      Node* dupePtr = head1;
      Node* current2 = head2;
      while (current1 != 0) // while not last node
      {
         unique = true;
         prev1 = current1;
         dupePtr = current1 -> link;
         while(dupePtr != 0)
         {
            if (current1 -> data == dupePtr -> data)
            {
               prev1 -> link = dupePtr -> link;
               if (head2 == 0)
               {
                  head2 = dupePtr;
                  current2 = head2;
            ...
            {
               prev1 = dupePtr;
               dupePtr = dupePtr -> link;
            }
         }
         if (unique == true)
         {
            Node *newNodePtr = new Node;
            newNodePtr -> data = current1 -> data;
            newNodePtr -> link = 0;
            if (head2 == 0)
            {
               head2 = newNodePtr;
